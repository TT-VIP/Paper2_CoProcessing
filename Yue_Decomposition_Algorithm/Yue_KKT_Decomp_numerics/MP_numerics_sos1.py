...
class KKTOCBlock:
    ...
    # primal slacks for inequalities (one per inequality)
...
class MasterProblem:
    ...
    def solve(self, *, time_limit: int = GRB.INFINITY, mip_gap: float = 1e-4) -> None:
        ...
        self.model.Params.IntFeasTol = 1e-6     # Default is 1e-5, can be tightened to 1e-6 for more precise integer solutions (at the cost of longer solve times)
        self.model.Params.PreSOS1BigM = 0   # Disable Gurobi's internal big-M handling for SOS1 constraints to ensure that our manually defined big-M values are used without modification (important for correctness of the SOS1-based reformulation of complementarity conditions)
        self.model.optimize()
    #endregion
    ...
    def _add_constraints(self) -> None:
        ...
        m.addConstrs(
            (gp.quicksum(self.d_siw[s,i,w] for i in data.I) == self.r_sw0[s,w] for s in data.S for w in data.W),
    ...
    def _add_kkt_oc_block_sos1(self, x_ck_fixed: Dict[Tuple[int, int], int]) -> int:
        ...
        # 3) Slack Variables for inequality constraints (SOS1) for this cut block
        ...
        m.addConstrs(
            (data.c_preproc_w[w] + data.c_truck*data.TD_sc[s][c] - gp.quicksum(data.phi_wh[w][h]*self.z_wh[w,h] for h in data.H) - lam_F3[c]*data.beta_w[w] 
        ...
        # F3: energy fulfillment
        # original: sum_f beta_f q_cf + sum_w beta_w q_scw >= alpha_c
        # slack_F3[c] = energy supplied - alpha_c >= 0
        m.addConstrs(
            (slack_F3[c] == gp.quicksum(q_cf[c, f] * data.beta_f[f] for f in data.F) 
             + gp.quicksum(q_scw[s, c, w] * data.beta_w[w] for s in data.S for w in data.W) - data.alpha_c[c] for c in data.C),
            name=f"{pfx}_slack_pf1_F3",
        )

        # F4: co-processing energy limit
        # original: sum_w beta_w q_scw <= kappa_coproc * alpha_c
        # slack_F4[c] = kappa_coproc * alpha_c - waste_energy >= 0
        m.addConstrs(
            (slack_F4[c] == data.kappa_coproc * data.alpha_c[c] 
             - gp.quicksum(q_scw[s, c, w] * data.beta_w[w] for s in data.S for w in data.W) for c in data.C),
            name=f"{pfx}_slack_pf2_F4",
        )

        # F5: investment capacity
        # original: sum_sw q_scw <= sum_k x_ck_fixed Q_k
        # slack_F5[c] = installed_capacity - used_capacity >= 0
        m.addConstrs(
            (slack_F5[c] == gp.quicksum(x_ck_fixed[(c, k)] * data.Q_k[k] for k in data.K)
             - gp.quicksum(q_scw[s, c, w] for s in data.S for w in data.W) for c in data.C),
            name=f"{pfx}_slack_pf3_F5",
        )

        # (pf4) F6 quota fulfillment (equality): sum_cw q_scw + sum_sw r_sw - mu_kiln*Q_gen_total = 0
        m.addConstr(
        ...
        for c in data.C:
            m.addSOS(GRB.SOS_TYPE1, [lam_F3[c], slack_F3[c]], [1.0, 2.0])
            m.addSOS(GRB.SOS_TYPE1, [lam_F4[c], slack_F4[c]], [1.0, 2.0])
            m.addSOS(GRB.SOS_TYPE1, [lam_F5[c], slack_F5[c]], [1.0, 2.0])
        ...
        #   pi_r_sw[s,w] ⟂ r_sw[s,w]
        for s in data.S:
            for w in data.W:
                m.addSOS(GRB.SOS_TYPE1, [pi_r_sw[s, w], r_sw[s, w]], [1.0, 2.0])
        ...
        lhs = (
            ...
            # Transport
            + data.c_truck * gp.quicksum(self.q_scw0[s, c, w] * data.TD_sc[s][c] for s in data.S for c in data.C for w in data.W)
        ...
        rhs = (
            ...
            # Transport
            + data.c_truck * gp.quicksum(q_scw[s, c, w] * data.TD_sc[s][c] for s in data.S for c in data.C for w in data.W)
            # Subsidy revenue (subtract)
