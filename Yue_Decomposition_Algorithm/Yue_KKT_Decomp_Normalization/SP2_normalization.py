...
from .MP_normalization import MasterSolution
from .SP1_normalization import SubProblem1Solution
...
class SubProblem2:
    ...
    def _add_constraints(self) -> None:
        ...
        m.addConstrs(
            (gp.quicksum(self.d_siw[s,i,w] for i in data.I) == self.r_sw[s,w] for s in data.S for w in data.W),
        ...
        # 5) Transportation cost to kilns
        self.obj_cost_transport = data.c_truck * gp.quicksum(self.q_scw[s,c,w] * data.TD_sc[s][c] for s in data.S for c in data.C for w in data.W)
        # 6) Subsidy revenue
        ...
        m.addConstr(
            self.obj_cost_coal + self.obj_cost_invest + self.obj_cost_preproc + self.obj_cost_penalty + self.obj_cost_transport - self.obj_revenue_subsidy <= self.sp1_optimal_value + 1e-6,  # small tolerance to account for numerical issues
        name="OptimalityConstraint"
        )

        self.obj_total_follower = self.obj_cost_coal + self.obj_cost_invest + self.obj_cost_preproc + self.obj_cost_penalty + self.obj_cost_transport - self.obj_revenue_subsidy
        #endregion
    ...
    def _set_objective(self) -> None:
        ...
        if self._has_normalization_bounds():
            ...
            self.obj_total_env_weighted_leader = data.weight_env * E_normalized
            self.obj_total_mon_weighted_leader = data.weight_mon * C_normalized
