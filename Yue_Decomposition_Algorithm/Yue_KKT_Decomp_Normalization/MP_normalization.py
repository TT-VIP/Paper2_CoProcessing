...
class MasterProblem:
    ...
    def _set_objective(self) -> None:
        ...
        if self._has_normalization_bounds():
            ...
            self.obj_total_env_weighted = data.weight_env * E_normalized
            self.obj_total_mon_weighted = data.weight_mon * C_normalized
    ...
    def _add_kkt_oc_block(self, x_ck_fixed: Dict[Tuple[int, int], int]) -> int:
        ...
        m.addConstrs(
            ...
            (r_sw[s, w] <= data.M_primal["r_sw"][s][w] * (1 - bin_r_sw[s, w]) for s in data.S for w in data.W),
