...
class MasterProblem:
    ...
    def build(self, *, name: str = "MasterProblem", output_flag: int = 1) -> None:
        ...
        if self._build: raise RuntimeError("Masterproblem model was already built.")
    ...
    def extract_solution(self) -> MasterSolution:
        ...
        if self.model.status == GRB.OPTIMAL:
            ...
            raise RuntimeError("Master Problem is not optimal; cannot extract solution.")
    ...
    def _add_constraints(self) -> None:
        ...
        # Precompute total generated waste (scalar) for readability
        Q_gen_total = gp.quicksum(data.Q_gw[g][w] for g in data.G for w in data.W)
        # (L7) Landfill transport must fulfill landfill quota
        m.addConstr(
            gp.quicksum(self.q_slw[s,l,w] for s in data.S for l in data.L for w in data.W) == self.mu_land * Q_gen_total,
        name="L7_landfillTransportQuota"
        )

        # (L8) Incinerator transport must fulfill incineration quota
        m.addConstr(
            gp.quicksum(self.q_siw[s,i,w] for s in data.S for i in data.I for w in data.W) == self.mu_inc * Q_gen_total,
