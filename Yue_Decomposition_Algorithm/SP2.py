...
class SubProblem2Solution:
    ...
    y_cwh: Dict[Tuple[int, int, int], float] | None    # Auxiliary variable for piecewise linear subsidy function (waste w at cement plant c in subsidy segment h)
...
class SubProblem2:
    ...
    It takes the leaders' optimal decisions from the master problem (MP) and optimal followers reaction value fro Subproblem 1 (SP1) to check
    ...
    def __init__(self, instance: InstanceData):
        ...
        self.q_cw = None  # Quantity of waste w processed at cement plant c
        ...
        self.y_cwh = None  # Auxiliary variable for piecewise linear subsidy function (waste w at cement plant c in subsidy segment h)
    ...
    def _add_variables(self) -> None:
        ...
        self.q_cw = m.addVars(data.C, data.W, lb=0.0, vtype=GRB.CONTINUOUS, name="q_cw")
        ...
        self.y_cwh = m.addVars(data.C, data.W, data.H, lb=0.0, vtype=GRB.CONTINUOUS, name="y_cwh")
    ...
    def _add_constraints(self) -> None:
        ...
        m.addConstr(
            gp.quicksum(data.phi_wh[w][h] * self.y_cwh[c,w,h] for c in data.C for w in data.W for h in data.H) <= data.budget_municipality,
        ...
        m.addConstr(
            cost_coal + cost_invest + cost_preproc + cost_penalty + cost_tiebreak - revenue_subsidy <= self.sp1_optimal_value,
    ...
    def _set_objective(self) -> None:
        '''Set objective function for SP2 (can be zero if only feasibility check, but for Yue logic equals leader objective to allow dual information for optimality cut generation)'''
        data = self.instance
        m = self.model
        ...
        emission_fuel = (
            ...
            gp.quicksum(data.epsilon_kiln_w[w] * self.q_cw[c,w] for c in data.C for w in data.W)
        )
