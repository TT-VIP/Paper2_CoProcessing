...
#        self.model.printQuality()  # Print solution quality information (e.g., MIP gap, bound, etc.) after solve
    ...
    def _f6_dual_strengthening_bounds_for_pattern(
        self,
        *,
        ...
        """
        Pattern-specific bounds used only in the F6 part of the primal-dual strengthening inequality.

        Motivation:
            The exact dual term contains -A_sw * lambda_F6[s,w]. Since lambda_F6[s,w] > 0 implies the F6 slack is zero,
            the relevant binding quantity is the accepted kiln flow from (s,w), not the gross station availability A_sw.

        Therefore, for strengthening we use

            U_F6_sw^l = min{
                U_A_sw,
                total installed pattern capacity,
                type-specific co-processing energy bound
            }.

        This bound is not used to replace primal feasibility; it is only used
        in the safe F6 dual contribution of the primal-dual strengthening cut.
        """
        data = self.instance

        total_pattern_capacity = float(sum(cap_c[c] for c in data.C))

        U_F6_sw = {}

        for s in data.S:
            for w in data.W:
                availability_bound = float(U_A_sw[(s, w)])

                # Type-specific upper bound from the co-processing energy limits:
                # sum_s q_scw[s,c,w] <= kappa_coproc * alpha_c / beta_w
                # aggregated over all cement plants c.
                energy_bound_w = float(sum(data.kappa_coproc * data.alpha_c[c] / data.beta_w[w] for c in data.C))

                U_F6_sw[(s, w)] = max(
                    0.0,
                    min(
                        availability_bound,
                        total_pattern_capacity,
                        energy_bound_w,
                    ),
                )
    ...
    def _dual_bound_expr_for_oc_block(
        ...
        """
        Dual lower-bound expression for an existing KKT-OC block.
        The F6 part uses pattern-specific strenghtening bounds U_F6_sw^l instead of the structural U_A_sw bounds.
        """
        data = self.instance
        ...
        U_F6_sw = self._f6_dual_strengthening_bounds_for_pattern(
            cap_c=cap_c,
            U_A_sw=U_A_sw,
        )
        ...
        return (
            ...
            - gp.quicksum(U_F6_sw[(s, w)] * oc.lam_F6[s, w] for s in data.S for w in data.W)
        )
    ...
    def add_dynamic_primal_dual_bound_cuts(
        ...
        *,
        ...
        for ell, oc in self.kkt_oc_blocks.items():
            cap_c = {c: float(sum(int(round(oc.x_ck_fixed[(c, k)])) * data.Q_k[k] for k in data.K)) for c in data.C}

            old_U_A = oc.constr.get("U_A_sw_used") if isinstance(oc.constr, dict) else None
            if old_U_A is None:
                old_U_A = self._structural_availability_bounds()

            old_U_F6 = oc.constr.get("U_F6_sw_used") if isinstance(oc.constr, dict) else None
            if old_U_F6 is None:
                old_U_F6 = self._f6_dual_strengthening_bounds_for_pattern(cap_c=cap_c, U_A_sw=old_U_A)

            new_U_F6 = self._f6_dual_strengthening_bounds_for_pattern(cap_c=cap_c, U_A_sw=U_A_sw)

            max_improvement = max(old_U_F6[(s, w)] - new_U_F6[(s, w)] for s in data.S for w in data.W)

            if max_improvement <= min_improvement:
                logging.info(
                    f"[U_F6_DYNAMIC_CUT] OC{ell}: no materially tighter U_F6 bound "
            ...
            oc.constr["U_F6_sw_used"] = dict(new_U_F6)
            cuts_added += 1

            logging.info(
                f"[U_F6_DYNAMIC_CUT] Added strengthened cut for OC{ell}: "
                f"{cname}, max U_F6 improvement={max_improvement:.6f}."
            )
    ...
    def _add_kkt_oc_block_sos1(self, x_ck_fixed: Dict[Tuple[int, int], int]) -> int:
        ...
        # Pattern-specific strengthening bound for the F6 dual term.
        # This is used only in the primal-dual strengthening inequality,
        # not in the primal feasibility definition of F6.
        U_F6_sw = self._f6_dual_strengthening_bounds_for_pattern(cap_c=cap_c, U_A_sw=U_A_sw)

        # check tightening via U_F6_sw vs structural U_A_sw
        max_f6_tightening = max(U_A_sw[(s, w)] - U_F6_sw[(s, w)] for s in data.S for w in data.W)
        sum_f6_tightening = sum(U_A_sw[(s, w)] - U_F6_sw[(s, w)] for s in data.S for w in data.W)

        logging.info(
            f"[{pfx}_U_F6] Pattern-specific F6 dual strengthening: "
            f"max tightening={max_f6_tightening:.6f}, "
            f"sum tightening={sum_f6_tightening:.6f}"
        )
        ...
        dual_lower_bound = (
            ...
            - gp.quicksum(U_F6_sw[(s, w)] * lam_F6[s, w] for s in data.S for w in data.W)
        )
        ...
        kkt_oc_block = KKTOCBlock(
            ...
            s_F5=s_F5,
            s_F6=s_F6,
            constr={"U_A_sw_used": dict(U_A_sw), "U_F6_sw_used": dict(U_F6_sw), "dynamic_pd_cuts": []}
        )
