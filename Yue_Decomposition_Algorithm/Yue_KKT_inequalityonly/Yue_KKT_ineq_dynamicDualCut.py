...
from .MP_ineq_SOS1_dynamicDualCut import MasterProblem as SOS1MasterProblem
...
def log_sos1_primal_dual_residuals(mp, data: InstanceData, *, tol: float = 1e-5) -> None:
    ...
    for ell, oc in mp.kkt_oc_blocks.items():
        ...
        # ------------------------------------------------------------
        if isinstance(getattr(oc, "constr", None), dict) and "U_F6_sw_used" in oc.constr:
            U_A_base = oc.constr["U_F6_sw_used"]
            U_A_source = "OC-specific U_F6_sw_used"
        elif isinstance(getattr(oc, "constr", None), dict) and "U_A_sw_used" in oc.constr:
...
def run_yue_decomposition(
    ...
    if sos1_cuts:
        logging.info("\nSolution Method: Complementarity conditions within the decomposition algorithm represented via SOS1 primal-dual cuts.")
    else:
        logging.info("\nSolution Method: Complementarity conditions within the decomposition algorithm represented via Big-M KKT OC cuts.")
